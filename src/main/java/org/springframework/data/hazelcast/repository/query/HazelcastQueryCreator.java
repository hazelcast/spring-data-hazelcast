...
public class HazelcastQueryCreator extends AbstractQueryCreator<KeyValueQuery<Predicate<?, ?>>, Predicate<?, ?>> {
    ...
    private Predicate<?, ?> from(Part part, Iterator<Comparable<?>> iterator) {
        ...
        switch (type) {
            ...
            case NOT_LIKE:
                ...
                String likeExpression = toLikeExpression(type, iterator.next().toString());
                Predicate likePredicate = ignoreCase ? Predicates.ilike(property, likeExpression) : Predicates.like(property, likeExpression);
            ...
            case SIMPLE_PROPERTY:
                return ifIgnoreCase(part) ? Predicates.ilike(property, iterator.next().toString()) : Predicates.equal(property, iterator.next());
            case NEGATING_SIMPLE_PROPERTY:
                return ifIgnoreCase(part) ? Predicates.not(Predicates.ilike(property, iterator.next().toString())) : Predicates.notEqual(property, iterator.next());
            case REGEX:
    ...
    private String toLikeExpression(Type type, String expression) {
        switch (type) {
            case CONTAINING:
            case NOT_CONTAINING:
