...
public class QueryIT extends TestDataHelper {
    ...
    public void findByFirstname() {
        ...
        assertThat("1940 and 1942", matches, 
        		containsInAnyOrder(hasProperty("lastname", equalTo("Cagney")),
        						   hasProperty("lastname", equalTo("Stewart"))));
    ...
    public void findByFirstname_with_sort() {
        ...
        assertThat("NULL sort", matches,
        		containsInAnyOrder(hasProperty("id", equalTo("1973")),
				   		   		   hasProperty("id", equalTo("1975")),
    ...
    public void findByLastnameIgnoreCase() {
        ...
        assertThat("1993 & 1994", matches,
        		containsInAnyOrder(hasProperty("id",equalTo("1993"))
        						  ,hasProperty("id",equalTo("1994")))
		);
	}
        ...
        try (Stream<Person> matches = this.personRepository.streamByLastnameGreaterThanEqual("Wayne")) {
            ...
            matches.forEach(match -> {
                ...
                assertThat("Wayne or Whitaker", match, 
                        anyOf( hasProperty("lastname", equalTo("Wayne"))
                              ,hasProperty("lastname", equalTo("Whitaker"))));
	...
	public void findByOrderByLastnameDesc() {
        ...
        while (pageResponse != null) {
            ...
            assertThat("Page " + pagesRetrieved + ", this page number",
                    pageResponse.getNumber(), equalTo(pagesRetrieved - 1));
            assertThat("Page " + pagesRetrieved + ", total page number",
                    pageResponse.getTotalPages(), equalTo(expectedNumberOfPages));

            assertThat("Page " + pagesRetrieved + ", item count v content",
                    pageResponse.getNumberOfElements(), equalTo(pageContent.size()));
            if (pagesRetrieved != expectedNumberOfPages) {
                assertThat("Page " + pagesRetrieved + ", item count",
                        pageResponse.getNumberOfElements(), equalTo(5));
            } else {
                assertThat("Page " + pagesRetrieved + ", item count",
                        pageResponse.getNumberOfElements(), greaterThan(0));
                assertThat("Page " + pagesRetrieved + ", item count",
                        pageResponse.getNumberOfElements(), lessThanOrEqualTo(SIZE_5));
            }
            assertThat("Page " + pagesRetrieved + ", total item count",
                    pageResponse.getTotalElements(), equalTo(new Long(Oscars.bestActors.length)));
